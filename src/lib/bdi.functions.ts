...
export const getLocationBDI = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ result: BDIResult; narrative: string }> => {
    const results = await Promise.all(
      TYPE_GROUPS.map((g) => nearbySearch(data.lat, data.lng, data.radius, g).catch(() => [])),
    );
    ...
    const narrative = await generateNarrative(data.locationName ?? "This location", result).catch(() => result.summary);
  ...
  try {
    const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are Found-r's location intelligence writer. Reply in 2-3 tight sentences (max 90 words) explaining WHY the Business Diversity Index score is high or low. No preamble. British English." },
          {
            role: "user",
